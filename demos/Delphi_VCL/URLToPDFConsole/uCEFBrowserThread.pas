...
type
  ...
  TCEFBrowserThread = class(TThread)
    protected
      ...
      FOnPDFPrintFinished    : TNotifyEvent;
      FErrorCode             : integer;
      FErrorText             : ustring;
      FFailedUrl             : ustring;
      FPendingUrl            : ustring;
      ...
      procedure DoOnPDFPrintFinished;
      procedure DetectContentSize;
      procedure Resize;
    ...
    public
      constructor Create(const aDefaultURL : ustring; aWidth, aHeight : integer; aDelayMs : integer = 500; const aScreenScale : single = 1; aOutputFormat: TOutputFormat = ofPNG; const aOutputPath: ustring = '');
      ...
      property OnPDFPrintFinished    : TNotifyEvent      read FOnPDFPrintFinished      write FOnPDFPrintFinished;
  end;
...
begin
  ...
  // Detect content size first
  if not FContentSizeDetected then
    DetectContentSize
  else
  begin
    // Content size has been detected, now generate output
    if FOutputFormat = ofPDF then
    begin
...
procedure TCEFBrowserThread.DoOnPDFPrintFinished;
begin
  if assigned(FOnPDFPrintFinished) then
    FOnPDFPrintFinished(self);
end;

procedure TCEFBrowserThread.DetectContentSize;
var
...
begin
  if not aResultOK then
    SetErrorText('PDF generation failed')
  else if assigned(FOnPDFPrintFinished) then
  begin
    if FSyncEvents then
      Synchronize(DoOnPDFPrintFinished)
    else
      DoOnPDFPrintFinished;
  end;
end;
